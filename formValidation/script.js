...
function checkInputs(arr) {
  ...
  for (input in arr) {
    ...
    if (arr[input].value.value == "") {
      ...
      (arr[input].name == "Username" && arr[input].value.value.length < 4) ||
      (arr[input].name == "Password" && arr[input].value.value.length < 4)
    ) {
      setError(arr[input].value, `${arr[input].name} must have 4 letters Max`);
      return;
    } else if (
      arr[input].name == "Username" &&
      !isNaN(arr[input].value.value.charAt(0))
    ) {
      setError(
        arr[input].value,
        `${arr[input].name} cannot start with a number`,
      );
      return;
    } else if (
      arr[input].name != "Password" &&
      !isNaN(arr[input].value.value.charAt(0)) &&
      arr[input].name != "Confirm Password"
    ) {
      setError(
        arr[input].value,
        `${arr[input].name} cannot start with a number`,
      ...
      arr[input].name == "Email" &&
      !emailRegex.test(arr[input].value.value)
    ) {
      ...
      arr[input].value.parentElement.querySelector("input").style.borderColor =
        "#2ecc71";
    }
  }
  resetErr(arr)
...
function resetErr(formInputs){
    ...
    element.value.parentElement.querySelector("input").style.borderColor = "";
